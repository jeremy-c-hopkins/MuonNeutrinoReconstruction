...
# Example usage:
# plot_stacked_1d_histograms(truth_cnn, reco_cnn, truth_cnn_bdt, reco_cnn_bdt,
#                            savefolder="path/to/save", variable_name="Inelasticity")


# Example usage:
# plot_1d_histogram_comparison_ratio(truth1, reco1, truth2, reco2, savefolder="path/to/save", variable_name="Inelasticity")


# Example usage:
# plot_1d_histogram_comparison(truth1, reco1, truth2, reco2, savefolder="path/to/save", variable_name="Inelasticity")
...
if __name__ == "__main__":
    ...
    # truth = np.loadtxt(
    #     "/home/bread/Documents/MuonNeutrinoReconstruction/src/data/reco/true.csv",
    #     delimiter=",",
    # )

    # reco = np.loadtxt(
    #     "/home/bread/Documents/MuonNeutrinoReconstruction/src/data/reco/reconstruction.csv",
    #     delimiter=",",
    # )

    # energy = np.loadtxt(
    #     "/home/bread/Documents/MuonNeutrinoReconstruction/src/data/reco/energy.csv",
    #     delimiter=",",
    # )

    # file = "/home/bread/Documents/MuonNeutrinoReconstruction/src/data/reco/comparison/oscNext_pisa_genie_1157_flercnn_plus.hdf5"

    # hdf = h5py.File(file, "r")

    # reco2 = np.array(hdf["numu"]["FLERCNN_plus_bdt_y"])
    # truth2 = np.array(hdf["numu"]["I3GENIEResultDict.y"])
    # energy2 = np.array(hdf["numu"]["MCInIcePrimary.energy"]) / 100

    # valid = np.where(truth2 > 0)
    # reco2 = reco2[valid]
    # truth2 = truth2[valid]
    # energy2 = energy2[valid]

    # valid_energy = np.where(energy2 <= 5.0)
    # reco2 = reco2[valid_energy]
    # truth2 = truth2[valid_energy]
    # energy2 = energy2[valid_energy]

    # print(min(reco), max(reco))

    # plot_2D_prediction_overlay(
    #     truth, reco, truth2, reco2, save=True, savefolder="src/data/plot"
    # )

    # neutrino_antineutrino_ratios()

    # error_against_energy_comparison(
    #     truth,
    #     reco,
    #     energy,
    #     truth2,
    #     reco2,
    #     energy2,
    #     "/home/bread/Documents/MuonNeutrinoReconstruction/src/data/plot",
    # )

    # plot_stacked_1d_histograms(
    #     truth,
    #     reco,
    #     truth2,
    #     reco2,
    ...
    # plot_2D_prediction_energy_ranges(
    #     truth,
    #     reco,
    #     energy,
    #     savefolder="/home/bread/Documents/MuonNeutrinoReconstruction/src/data/plot",
    # )
    ...
    # for file in files:
    #     hdf = h5py.File(file, "r")
    #     inelasticity.extend(np.array(hdf["Y_train"][:, 14]))
    #     inelasticity.extend(np.array(hdf["Y_test"][:, 14]))
    #     inelasticity.extend(np.array(hdf["Y_validate"][:, 14]))
    # anti.extend(np.array(hdf["Y_train"][:, 10]))

    # plot_inelasticity_distribution(
    #     inelasticity,
    #     save=True,
    #     savefolder="/home/bread/Documents/MuonNeutrinoReconstruction/src/data/plot",
    # )
    ...
    # df = pd.DataFrame({"Inelasticity": inelasticity, "IsAntineutrino": anti})
    ...
    color1 = cmap(0.2)
    color2 = cmap(0.8)
    kde_anti = stats.gaussian_kde(anti["Inelasticity"])
    kde_neutrino = stats.gaussian_kde(neutrino["Inelasticity"])
    x_range = np.linspace(
        min(anti["Inelasticity"].min(), neutrino["Inelasticity"].min()),
        max(anti["Inelasticity"].max(), neutrino["Inelasticity"].max()),
        1000,
    )
    ax.plot(x_range, kde_anti(x_range), label="Antineutrino", color=color1)
    ax.plot(x_range, kde_neutrino(x_range), label="Neutrino", color=color2)
    ax.set_title("Neutrino vs Antineutrino Inelasticity - 1d Histogram", fontweight=400)
    ax.set_xlabel("Inelasticity Range")
    ax.legend()
    ax.set_ylabel("Occurence")
    ax.grid("on", alpha=0.2, linestyle="-")
    plt.show()
    ...
    ax.hist(
        anti["Inelasticity"],
        bins=50,
        label=f"Antineutrino Inelasticity",
        alpha=0.6,
        color="olive",
    )
    ax.hist(
        neutrino["Inelasticity"],
        bins=50,
        label=f"Neutrino Inelasticity",
        alpha=0.6,
        color="deepskyblue",
    )
    # fig.suptitle('Deep Neural Network', fontweight=600)
    ax.set_title(f"Neutrino vs Antineutrino Inelasticity - 1d Histogram")
    ax.grid("on", alpha=0.2, linestyle="-")
    plt.legend()
    plt.show()
    plt.close()
